				...
				<!--trangchinhgiua-->
					<div class="agileinfo-grap">
						<div class="agileits-box">
							...
							<h4 style = "color : blue"> <strong> Thông Tin Lịch Thực Hành Khoa CNTT </strong></h4>
							<?php 
							
								$malth = $malop= $mamh =$magv = $maphong = $matg = $thu = $ngaybatdau = $sotuanthuchanh  =  "";
								// nếu tồn tại ok thì check điều kiện
								if(isset($_POST['ok'])){
									if($_POST['malth'] == ""){
										$errormalth = "Vui lòng nhập lại mã lịch thực hành !";
									}else{
										$malth = $_POST['malth'];
									}
									if($_POST['malop'] == ""){
										$errormalop = "Vui lòng nhập lại mã lớp !";
									}else{
										$malop = $_POST['malop'];
									}
									
									if($_POST['mamh'] == ""){
										$errormamh = "Vui lòng nhập lại mã môn học !";
									}else{
										$mamh = $_POST['mamh'];
									}
									
									if($_POST['magv'] == ""){
										$errormagv = "Vui lòng nhập lại mã giáo viên !";
									}else{
										$magv = $_POST['magv'];
									}
									
									if($_POST['maphong'] == ""){
										$errormaphong = "Vui lòng nhập lại mã phòng !";
									}else{
										$maphong = $_POST['maphong'];
									}
									if($_POST['matg'] == ""){
										$errormatg = "Vui lòng nhập lại mã thời gian !";
									}else{
										$matg = $_POST['matg'];
									}
									if($_POST['thu'] == ""){
										$errorthu = "Vui lòng nhập lại bạn học thứ mấy !";
									}else{
										$thu = $_POST['thu'];
									}
									
									if($_POST['ngayhoc'] == ""){
										$errorngaybatdau = "Vui lòng nhập lại ngày sinh giáo viên !";
									}else{
										$ngaybatdau = $_POST['ngayhoc'];
									}
									
									
									if($_POST['sotuan'] == ""){
										$errorsotuanthuchanh = "Vui lòng nhập lại số tuần thực hành !";
									}else{
										$sotuanthuchanh = $_POST['sotuan'];
									}
									...
									while($row = mysql_fetch_array($rs, MYSQL_BOTH)){
										if($malth == $row['MaLTH']){
											echo '<span style = "color : red"> "Mã lịch học đã tồn tại, vui lòng nhập lại mã lịch học khác ..."</span>';
											break;
										}else {
											if( $malth && $malop  && $mamh && $magv && $maphong && $matg && $thu && $ngaybatdau && $sotuanthuchanh  ){
												$sqlinsert = "INSERT INTO  lichthuchanh 
												values ( '".$malth."' , '".$malop."' , '".$mamh."', '".$magv."', 
												'".$maphong."', '".$matg."' , '".$thu."', '".$ngaybatdau."'  , '".$sotuanthuchanh."' )";
												mysql_query($sqlinsert);				
												header("location:SuaxemthongtinlichTH.php");
											}
										}
							...
							?>
								<form style = "color : blue " action="" method = "POST">
									<br><br>
											<table>
												<tr> 
													<td><span>Mã Lịch Thực Hành :</span></td>
													<td><input type="text" name ="malth" value = "LTH0"/></td>
													<td style ="color:red"><?php echo isset($errormalth) ? $errormalth : "" ?></td>
												</tr>
												<tr> 
													<td><span>Mã Lớp :</span></td>
													<td><input type="text" name ="malop" value = "N"/></td>
													<td style ="color:red"><?php echo isset($errormalop) ? $errormalop : "" ?></td>
												</tr>
												<tr> 
													<td><span>Mã Môn Học :</span></td>
													<td><input type="text" name ="mamh" value = ""/></td>
													<td style ="color:red"><?php echo isset($errormamh) ? $errormamh : "" ?></td>
												</tr>
												<tr> 
													<td><span>Mã Giao Viên :</span></td>
													<td><input type="text" name ="magv" value = "GV0"/></td>
													<td style ="color:red"><?php echo isset($errormagv) ? $errormagv : "" ?></td>
												</tr>
												<tr> 
													<td><span>Mã Phòng Học :</span></td>
													<td><input type="text" name ="maphong" value = "MP"/></td>
													<td style ="color:red"><?php echo isset($errormaphong) ? $errormaphong : "" ?></td>
												</tr>
												<tr> 
													<td><span>Mã Thời Gian :</span></td>
													<td><input type="text" name ="matg" value = "MTG0"/></td>
													<td style ="color:red"><?php echo isset($errormatg) ? $errormatg : "" ?></td>
												</tr>
												<tr> 
													<td><span>Thứ  :</span></td>
													<td><input type="text" name ="thu" value = ""/></td>
													<td style ="color:red"><?php echo isset($errorthu) ? $errorthu : "" ?></td>
												</tr>
												
												<tr> 
													<td><span>Ngày Bắt Đầu Học : </span></td>
													<td><input type="date" name = "ngayhoc"value = ""/></td>
													<td style ="color:red"><?php echo isset($errorngaybatdau) ? $errorngaybatdau : "" ?></td>
												</tr>
												<tr> 
													<td><span>Số Tuần Thực Hành : </span></td>
													<td><input type="number" name = "sotuan" value = ""/></td>
													<td style ="color:red"><?php echo isset($errorsotuanthuchanh) ? $errorsotuanthuchanh : "" ?></td>
												</tr>
												<tr> 
													<td colspan="3">
			...
			<div class="sidebar-menu">
				...
				<div class="menu">
					<ul id="menu" >
						...
						<li id="menu-academico" ><a href="XoaxemGV.php"><i class="fa fa-cogs" aria-hidden="true"></i><span>Xóa Thông Tin</span> <span class="fa fa-angle-right" style="float: right"></span><div class="clearfix"></div></a>
							<ul id="menu-academico-sub" >
