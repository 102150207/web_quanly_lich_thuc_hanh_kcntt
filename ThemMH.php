				...
				<!--trangchinhgiua-->
					<div class="agileinfo-grap">
						<div class="agileits-box">
							...
							<h4 style = "color : blue"> <strong> Thông Tin Môn Thực Hành Khoa CNTT </strong></h4>
							<?php 
								 $mamh = $tenmh = $sotc  =  "";
								...
								if(isset($_POST['ok'])){
									...
									if($_POST['tenmh'] == ""){
										$errorten = "Vui lòng nhập lại tên môn học !";
									}else{
										$tenmh = $_POST['tenmh'];
									}
									if($_POST['sotc'] == ""){
										$errorsotc = "Vui lòng nhập lại số tín chỉ !";
									}else{
										$sotc = $_POST['sotc'];
									}
									...
									while($row = mysql_fetch_array($rs, MYSQL_BOTH)){
										if($mamh == $row['MaMH']){
											echo '<span style = "color : red"> "Mã Môn học đã tồn tại, vui lòng nhập lại mã môn học khác ..."</span>';
											break;
										}else {
											if( $mamh && $tenmh && $sotc){
												$sqlinsert = "INSERT INTO  monhoc 
												values ( '".$mamh."' , '".$tenmh."' , '".$sotc."' )";
												mysql_query($sqlinsert);				
												header("location:Suaxemthongtinmonhoc.php");
											}
										}
							...
							<form style = "color : blue " action="" method = "POST">
								<br><br>
										<table>
											...
											<tr> 
												<td><span>Số Tín Chỉ : </span></td>
												<td><input type="text" name = "sotc" value = ""/></td>
												<td style ="color:red"><?php echo isset($errorsotc) ? $errorsotc : "" ?></td>
											</tr>
											<tr> 
												<td colspan="3">
