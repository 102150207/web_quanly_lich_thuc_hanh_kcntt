					...
					<div class="agileinfo-grap">
						<div class="agileits-box">
							...
							<?php
								...
								$pt = $_REQUEST['select_name'];
								$data = $_REQUEST['t5'];
								$i=1;
								echo'i';
									if($data =="")
								{
									header("location:Timthuchanh.php");
								} else if($pt == "t1") {
										$link  = mysql_connect("localhost","root","") or die ("Khong the ket noi CSDL");
										//$db_selected = ;
										mysql_select_db("final",$link);
										$sql = "select * from lichthuchanh  ";
										$result  = mysql_query("SELECT * FROM lichthuchanh inner join lop on lichthuchanh.MaLop = lop.MaLop
										...
										inner join thoigian on lichthuchanh.MaTG = thoigian.MaTG
										where MaLTH ='".$data."' or monhoc.TenMH = '".$data."' "); 
										
										if(mysql_num_rows($result) == 0){
										header("location:Timthuchanh.php");
										}else {
										echo '<table style = "color : blue " border ="1" width = "100%">';
										echo '<h1><caption> Danh Sach Tim Kiem  </caption></h1>';
										echo '<tr><th>STT </th><th>Ma Lich </th><th>Mon Hoc </th><th>Giao vien</th><th>Lop</th>
										<th>Phong</th><th>Thu</th><th>Tiet</th><th>Thoi Gian Bat Dau</th><th>So Tuan Thuc Hanh</th></tr>';
										$i = 1;
									while($row1 = mysql_fetch_array($result, MYSQL_BOTH)){
								echo '<tr><td>'.$i.'</td> 
											<td>'.$row1['MaLTH'].'</td><td>'.$row1['TenMH'].'</td><td>'.$row1['TenGV'].'</td>
											<td>'.$row1['TenLop'].'</td> <td>'.$row1['TenPHG'].'</td><td>'.$row1['Thu'].'</td>
											<td>'.$row1['Tiet'].'</td><td>'.$row1['NgayBatDau'].'</td><td>'.$row1['SoTuanThucHanh'].'</td>
											</tr>';
									$i++;
									}
								echo '</table>';
								...
								/* } else if($pt = "t2"){
									$link  = mysql_connect("localhost","root","") or die ("Khong the ket noi CSDL");
										...
										$result  = mysql_query("SELECT * FROM giaovien Where DiaChi = '".$data."' "); 
										
										if(mysql_num_rows($result) == 0){
										header("location:FormTimGV.php");
										}else {
										echo '<table border ="1" width = "100%">';
										echo '<h1><caption> Danh Sach Tim Kiem  </caption></h1>';
										echo '<tr><th>STT </th><th>Ho Ten</th><th>Ma So</th>
										<th>Thong tin chi tiet</th></tr>';
										$i = 1;
									while($row1 = mysql_fetch_array($result, MYSQL_BOTH)){
								echo '<tr><td>'.$i.'</td> 
											<td>'.$row1['TenGV'].'</td> <td>'.$row1['MaGV'].'</td> 
											<td><a href="DsTimGV.php?id='.$row1['MaGV'].'">xxx</a></td></tr>';
									$i++;
									}
								echo '</table>';
