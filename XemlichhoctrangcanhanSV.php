				...
				<!--trangchinhgiua-->
					<div class="agileinfo-grap">
						<div class="agileits-box">
							...
							<h4 style = "color : blue"> <strong> Thông Tin Tất Cả Các Giao Viên Khoa CNTT </strong></h4>
							<?php 
								session_start();
								$s = $_SESSION['id'];
								$link = mysql_connect("localhost","root","") or die ("Khong the ket noi CSDL");
								mysql_select_db("final",$link);
								$sql = "SELECT monhoc.TenMH, giaovien.TenGV, phong.TenPHG,
								thoigian.Tiet, lichthuchanh.Thu, lichthuchanh.NgayBatDau, lichthuchanh.SoTuanThucHanh
								FROM sinhvien inner join lichthuchanh on sinhvien.MaLop = lichthuchanh.MaLop
								inner join lop on lop.MaLop = sinhvien.MaLop
								inner join phong on lichthuchanh.MaPHG= phong.MaPHG
								inner join monhoc on lichthuchanh.MaMH = monhoc.MaMH
								inner join giaovien on lichthuchanh.MaGV = giaovien.MaGV
								inner join thoigian on thoigian.MaTG= lichthuchanh.MaTG
								where sinhvien.MaSV = '".$s."'";
								$rs = mysql_query($sql,$link);
								
							echo '<table style = "color : blue " border ="1" width = "100%">';
									echo '<h1><caption> Lich Hoc cua Sinh Vien </caption></h1>';
									echo '<tr><th>Mon Hoc </th><th>Giao Vien</th><th>Phong</th>
									<th>Tiet</th><th>Thu</th><th>Ngay Bat Dau</th><th>So Tuan Thuc Hanh</th></tr>';
								while($row = mysql_fetch_array($rs, MYSQL_BOTH)){
							echo '<tr><td>'.$row['TenMH'].'</td> <td>'.$row['TenGV'].'</td> 
										<td>'.$row['TenPHG'].'</td> <td>'.$row['Tiet'].'</td>
										<td>'.$row['Thu'].'</td><td>'.$row['NgayBatDau'].'</td> 
										<td>'.$row['SoTuanThucHanh'].'</td></tr>';
								}
							echo '</table>';
			...
			<div class="sidebar-menu">
				...
				<div class="menu">
					<ul id="menu" >
						...
						<li id="menu-academico" ><a href="trangcanhanSV.php"> <i class="fa fa-cogs" aria-hidden="true"></i><span>Xem Thông Tin cá Nhân</span> </a>
						
						</li>
						<li id="menu-academico" ><a href="XemlichhoctrangcanhanSV.php"> <i class="fa fa-cogs" aria-hidden="true"></i><span>Xem Lịch Học</span> </a>
						
						</li>
						<li id="menu-academico" ><a href="SuatrangcanhanSV.php"> <i class="fa fa-cogs" aria-hidden="true"></i><span>Sửa Thông Tin Cá Nhân</span> </a>
