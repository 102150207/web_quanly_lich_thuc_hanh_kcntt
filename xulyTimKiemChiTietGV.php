					...
					<div class="agileinfo-grap">
						<div class="agileits-box">
							...
							<?php 
									...
									$sql = "SELECT * FROM giaovien WHERE MaGV = '".$s."'";
									$rs = mysql_query($sql,$link);
									$row = mysql_fetch_assoc($rs);
									echo '<table  style = "color : blue " border ="1" width = "100%">';
										echo '<h1><caption> Du Lieu truy xuat tu bang Giao Vien </caption></h1>';
										echo '<tr><th>Ma So</th><th>Ho Ten</th><th>Gioi Tinh</th>
										<th>Ngay Sinh</th><th>Dia Chi</th><th>So Dien Thoai</th></tr>';
											echo '<tr><td>'.$s.'</td> <td>'.$row['TenGV'].'</td>
											<td>'.$row['GioiTinh'].'</td>
											<td>'.$row['NgaySinh'].'</td><td>'.$row['DiaChi'].'</td>
											<td>'.$row['SoDT'].'</td> 
								...
								<?php 
									...
									$link = mysql_connect("localhost","root","") or die ("Khong the ket noi CSDL");
									mysql_select_db("final",$link);
									$sql = "SELECT monhoc.TenMH, lichthuchanh.MaLop, phong.TenPHG,
									thoigian.Tiet, lichthuchanh.Thu, lichthuchanh.NgayBatDau, lichthuchanh.SoTuanThucHanh
									FROM lichthuchanh inner join giaovien on lichthuchanh.MaGV = giaovien.MaGV
									inner join lop on lop.MaLop = lichthuchanh.MaLop
									...
									where lichthuchanh.MaGV = '".$s."'";
									$rsx = mysql_query($sql,$link);
									
								echo '<table  style = "color : blue " border ="1" width = "100%">';
										echo '<h1><caption> Lich Giang Day  </caption></h1>';
										echo '<tr><th>Mon Hoc </th><th>Phong</th>
										<th>Tiet</th><th>Thu</th><th>Ngay Bat Dau</th><th>So Tuan Thuc Hanh</th></tr>';
										
									while($row1 = mysql_fetch_array($rsx, MYSQL_BOTH)){
								echo '<tr><td>'.$row1['TenMH'].'</td> 
											<td>'.$row1['TenPHG'].'</td> <td>'.$row1['Tiet'].'</td>
											<td>'.$row1['Thu'].'</td><td>'.$row1['NgayBatDau'].'</td> 
											<td>'.$row1['SoTuanThucHanh'].'</td> </tr>';
